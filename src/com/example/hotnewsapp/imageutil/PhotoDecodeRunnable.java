...
package com.example.hotnewsapp.imageutil;
 ...
 * which implements the methods {@link TaskRunnableDecodeMethods}. PhotoTask
 * objects call {@link #PhotoDecodeRunnable(TaskRunnableDecodeMethods)
 * PhotoDecodeRunnable()} with themselves as the argument. In effect, an
 * PhotoTask object and a PhotoDecodeRunnable object communicate through the
...
class PhotoDecodeRunnable implements Runnable {
	...
	interface TaskRunnableDecodeMethods {
		...
		/**
		 * Returns the desired width of the image, based on the ImageView being
		 * created.
		 * 
		...
		/**
		 * Returns the desired height of the image, based on the ImageView being
		 * created.
		 * 
	 ...
	 * 
	 * @param downloadTask
	 *            The PhotoTask, which implements ImageDecoderRunnableCallback
	 */
	...
	public void run() {
		...
		try {
			...
			/*
			 * Calls the PhotoTask implementation of {@link #handleDecodeState}
			 * to set the state of the download
			 */
			...
			/*
			 * Sets the desired image height and width based on the ImageView
			 * being created.
			 */
			...
			/*
			 * Even if the decoder doesn't set a Bitmap, this flag tells the
			 * decoder to return the calculated bounds.
			 */
			bitmapOptions.inJustDecodeBounds = true;

			/*
			 * First pass of decoding to get scaling and sampling parameters
			 * from the image
			 */
			...
			/*
			 * Sets horizontal and vertical scaling factors so that the image is
			 * expanded or compressed from its actual size to the size of the
			 * target ImageView
			 */
			...
			/*
			 * Sets the sample size to be larger of the horizontal or vertical
			 * scale factor
			 */
			//
			...
			/*
			 * If either of the scaling factors is > 1, the image's actual
			 * dimension is larger that the available dimension. This means that
			 * the BitmapFactory must compress the image by the larger of the
			 * scaling factors. Setting inSampleSize accomplishes this.
			 */
//			if (sampleSize > 1) {
...
//			}
			...
			/*
			 * This does the actual decoding of the buffer. If the decode
			 * encounters an an out-of-memory error, it may throw an Exception
			 * or an Error, both of which need to be handled. Once the problem
			 * is handled, the decode is re-tried.
			 */
			for (int i = 0; i < NUMBER_OF_DECODE_TRIES; i++) {
				try {
					 ...
					 * break;
					 * 
					 * /* If the decode fails, this block tries to get more
					 * memory.
					 */
				} catch (Throwable e) {
					...
					/*
					 * Tells the system that garbage collection is necessary.
					 * Notice that collection may or may not occur.
					 */
					java.lang.System.gc();
					...
					/*
					 * Tries to pause the thread for 250 milliseconds, and
					 * catches an Exception if something tries to activate the
					 * thread before it wakes up.
					 */
					try {
